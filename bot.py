...
async def set_afk(interaction: discord.Interaction, status: str = None):
    ...
    if user_id in afk_data and not status:
        ...
        msg = await interaction.followup.send(f"✅ {interaction.user.mention}, you are no longer AFK!")
        ...
        save_afk_data(afk_data)
        msg_text = f"✅ {interaction.user.mention}, you are now AFK"
        if status:
            msg_text += f": {status}"
        msg = await interaction.followup.send(msg_text)
        await asyncio.sleep(3)
        try:
            await msg.delete()
        except:
...
async def unclear_channel(interaction: discord.Interaction):
    ...
    try:
        ...
        restore_embed = discord.Embed(
            ...
            description=f"Restoring {len(saved_messages)} messages...",
            color=discord.Color.blue()
        )
        restore_embed.set_footer(text=FOOTER_TEXT, icon_url=LOGO_URL)
        await channel.send(embed=restore_embed)
        
        for msg_data in saved_messages:
            try:
                msg_embed = discord.Embed(
                    description=msg_data["content"] if msg_data["content"] else "*[No text content]*",
                    color=discord.Color.greyple(),
                    timestamp=datetime.fromisoformat(msg_data["timestamp"])
                )
                msg_embed.set_author(name=msg_data["author_display_name"])
                
                if msg_data.get("attachments"):
                    attachments_text = "\n".join([f"📎 {url}" for url in msg_data["attachments"]])
                    msg_embed.add_field(name="Attachments", value=attachments_text[:1024], inline=False)
        ...
        success_embed = discord.Embed(
            ...
            description=f"Successfully restored {len(saved_messages)} messages!",
            color=discord.Color.green()
        )
        success_embed.set_footer(text=FOOTER_TEXT, icon_url=LOGO_URL)
...
async def user_info(interaction: discord.Interaction, user: discord.Member = None):
    ...
    has_only_member_role = len([r for r in interaction.user.roles if r.id != interaction.guild.default_role.id]) == 1 and MEMBER_ROLE_ID in user_roles
    
    if has_only_member_role:
        await interaction.response.send_message("❌ You don't have permission to use this command!", ephemeral=True)
        return
