...
class TicketManagementView(discord.ui.View):
    ...
    async def claim_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # --- NEW LOCKOUT LOGIC: PREVENT OTHER STAFF FROM TYPING ---
        for rid in STAFF_ROLE_IDS:
            r = interaction.guild.get_role(rid)
            if r:
                # Set others to Read-Only (Cannot Send)
...
# --- NEW UNCLAIM COMMAND ---
@bot.tree.command(name="unclaim", description="Release a ticket so others can claim it", guild=discord.Object(id=GUILD_ID))
@app_commands.check(is_middleman_or_above)
async def unclaim_ticket(interaction: discord.Interaction):
    ...
    # Unlock Staff Roles (Reset Permissions)
    overwrites = interaction.channel.overwrites
    
    # Define admin roles here to avoid NameError if not globally accessible in scope (though they are global in this file)
    admin_full_access_roles = [OWNER_ROLE_ID, CO_OWNER_ROLE_ID, ADMINISTRATOR_ROLE_ID]

    for role_id in STAFF_ROLE_IDS:
        role = interaction.guild.get_role(role_id)
        if role:
            # Check what permission they should have based on ticket type
            # Support tickets: Send Messages TRUE
            # Middleman tickets: Send Messages FALSE (Read Only)
            
            can_send = False
            if ticket_type_found == "user_support_tickets":
                can_send = True
            
            # Admins always get send access if defined in creation logic, keeping consistent
            if role_id in admin_full_access_roles:
                can_send = True

            await interaction.channel.set_permissions(role, view_channel=True, send_messages=can_send)
 
    # Remove specific user override for the person who claimed it
    await interaction.channel.set_permissions(interaction.user, overwrite=None)
 
    embed = discord.Embed(
        title="🔓 Ticket Unclaimed", 
        description=f"This ticket has been released by {interaction.user.mention}.", 
        color=discord.Color.orange()
    )
